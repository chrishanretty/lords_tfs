...
here::i_am("R/401_aggregate_second_readings.R")

second <- read.csv(here::here("working",
                              "second_readings_checked.csv")) |>
    dplyr::select(-notes, -already_checked)

aux <- read.csv(here::here("working",
                           "second_readings_manual_garnished.csv"))

second <- rbind(second, aux)
...
### Date handling
    df <- df |>
        mutate(date2 = gsub("[A-Za-z]", "", date2),
               date2 = as.Date(date2))
...
saveRDS(res, file = here::here("outputs", "agg_second_readings.rds"))
